...
for i in range(int(6*6*300*7)):
    ...
    T_iter=next_T(T_iter)
    if Tmax[-1]>80+T0:
        print('在 '+str(i)+' 步超过80度！')
        break
    pass
Tmax=np.array(Tmax)-T0
print(T_iter.max()-T0)
...
plt.plot(ddd,TTTconst,label='$T_{final}=$'+str(int(TTT[-1]))+'$\degree C$')
plt.legend(fontsize='x-large')
plt.grid(True)
plt.title('$T_{max}$ with Days When Number=160')
plt.xlabel('t / Days')
plt.ylabel('$T_{max}$ / $\degree C$')
plt.savefig('160.png',format='png')

import seaborn as sns
sns.heatmap(T_iter.T-T0,cmap='RdBu_r')
plt.title('Stable Temperature Distribution\n When Number=170 at Day 88\n $T_{max}=76\degree C$')
plt.axis('off')
plt.savefig('160T.png',format='png')