...
def vt(ti):
    tm=ti*100/60/60/24/365*12
    return v+0.5*np.sin(2*np.pi/12*tm)
def Tenvt(ti):
    tm=ti*100/60/60/24/365*12
    return Tenv+5*np.sin(2*np.pi/12*tm)
...
def next_ji_t(j,i,Tnow,ti):
    ...
    if edge(j,i) =='in':
        ...
        Tji_next=Tji+dt*(K*T22-vnow*Tz_/c/rho)
        pass
    elif edge(j,i)=='on-side':
        Tji_next=Tji+dt*(Ka*T22+qs-vnow*Tz_/c/rho)
        pass
    elif edge(j,i)=='on-updown':
        Tji_next=Tji+dt*(Ka*T22+qs)
        pass
...
def next_T_t(Tnow,ti):
    T_next=np.zeros([M,N])+Tenvt(ti)+T0
    for j in range(1,M-1):
        for i in range(1,N-1):
            T_next[j][i]=next_ji_t(j,i,Tnow,ti)
            pass
        pass
    return T_next
    pass